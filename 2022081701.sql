...
3) LPAD(C1,n[,c2]), RPAD(c1,n[,c2]) - **
  - LPAD : 주어진 문자열 c1을 지정된 기억공간의 n에 오른쪽부터 저장하고 남는 공간에
           c2 문자열을 삽입함. 단 c2가 생략되면 공백을 삽입 
           수표보호문자로 주로 사용
           
  - RPAD : 주어진 문자열 c1을 지정된 기억공간의 n에 오른쪽부터 저장하고 남는 공간에
           c2 문자열을 오른쪽에 삽임함. 단 c2가 생략되면 공백을 삽입
...
사용예)회원테이블에서 마일리지가 많은 회원 3명이 2020년 4-6월 구매한 정보를 
      조회하시오.
      조회할 컬럼은 회원번호, 회원명, 마일리지, 구매금액합계이다.
