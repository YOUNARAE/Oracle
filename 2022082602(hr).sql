...
department_id=180;
update hr.departments set department_name='계약팀' where
department_id=190;
...
사용예)
 ...
 FROM bom --제품 리스트를 bom이라고 한다
 START WITH parent_id IS NULL --루트노드(제일 상위에 있는)를 지정, 노드는 행이라고 친다
 CONNECT BY PRIOR item_id = parent_id;--부모와 / 자식노드들간의 관계를 연결 , PRIOR는 부모서부터 자식까지 검색해나가는 방법 자식 컬럼값=부모컬럼값


 SELECT LPAD(' ', 2*(LEVEL-1)) || item_name as item_names, --LPAD(' ', 2*(LEVEL-1)) 왼쪽에 공간을 인쇄해주는 것
 item_id,
 parent_id
 FROM bom
 ...
 select level, --출력했을 때, NULL값을 가지고 있는 게 상위부서코드임
